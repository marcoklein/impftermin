...
export async function bookAppointment(page: Page, queueEntry: QueueElement) {
  ...
  if (
    !queueEntry.title ||
    !queueEntry.firstname ||
    !queueEntry.lastname ||
    !queueEntry.zip ||
    !queueEntry.city ||
    !queueEntry.street ||
    !queueEntry.streetnumber ||
    !queueEntry.mobile ||
    !queueEntry.email ||
    !queueEntry.earliestdate ||
    !queueEntry.latestdate
  ) {
  ...
  const desiredEarliestLinearDate: number =
    parseInt(desiredEarliestDate.substring(0, 2)) +
    parseInt(desiredEarliestDate.substring(3, 5)) * 31;
  ...
  const desiredLatestLinearDate: number =
    parseInt(desiredLatestDate.substring(0, 2)) +
    parseInt(desiredLatestDate.substring(3, 5)) * 31;
  ...
  for (const listElementTermin of await page.$$("span")) {
    ...
    if (idName && idName.includes(".,")) {
      ...
      if (dateNumber % 2 !== 0) {
        ...
        linearDate =
          parseInt(idName.substring(5, 7)) +
          parseInt(idName.substring(8, 10)) * 31;
        if (
          linearDate >= desiredEarliestLinearDate &&
          linearDate <= desiredLatestLinearDate
        ) {
    ...
    if (idName && idName.includes("Uhr")) {
      appointmentDates[dateNumber] = appointmentDates[dateNumber].concat(
        idName
      );
  ...
  if (!desiredEarliestDateFound) {
    ...
    debug(
      "Latest available date for first shot is %s",
      appointmentDates[dateNumber - 1]
    );
